...
/**
 * Optimistic persistence-sync primitive shared by the estimate calculator and
 * the quote builder.
 ...
 *  - `runNow(thunk, revert)` fires immediately (add / duplicate / delete / reorder).
 *  - Both retry network failures with exponential backoff (~3 attempts) before
 *    giving up. A handled `{ ok:false }` (NotFound/validation) does NOT retry.
 *    On give-up or `{ ok:false }`: call `revert()` + toast.
 *  - `saving` is true while any request is in flight (drives the "Saving…" hint).
 *  - `failMessage` is the toast shown on failure (the caller passes its localized
 *    string, so this hook is namespace-agnostic).
 * Last-write-wins; no version column (known limitation).
 */
export function useRecordSync(failMessage: string) {
  ...
  // Structure changes run through a serial queue so they commit one-at-a-time in
  // call order. This keeps lazy "create on first write" rows from racing (N
  // concurrent first-adds would otherwise each create a parent), and preserves
  // append order. exec never rejects (it catches + reverts), so the chain can't
  // break. Returns the settle promise so callers can await ordering when needed.
  const queue = React.useRef<Promise<void>>(Promise.resolve());
  const runNow = React.useCallback(
    (thunk: Thunk, revert: () => void) => {
      queue.current = queue.current.then(() => exec(thunk, revert));
